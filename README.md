...
[Cambridge Journal for Junior Scientists (CJJS)](http://www.intlpress.com/CJJS) articles.

## Package content

The following files are given in the repository (or directly in `*.zip` archive):

* `ipart.cls`, `ipart-layout-cjjs.sty` - LaTeX style files designed for International Press journal articles.
...
* `cjjs-template.tex` - topmatter template (should be used for article preparation);
* `cjjs-sample.tex` - journal sample article;
* `cjjs-sample.pdf` - journal sample article (`PDF` file);
...
* Install `ipart.cls`, `ipart-layout-cjjs.sty`, `imsart-number.bst`, `imsart-nameyear.bst` in your TeX system (suggested directory: `ipart`).
...
Use `cjjs-template.tex` as a template.

### Document class options

For the CJJS journal `cjjs` option must be set
...
```latex
\documentclass[cjjs,authoryear]{ipart}
```
...
```latex
\documentclass[cjjs,nonatbib]{ipart}
```
...
[github](https://github.com/vtex-soft/texsupport.intlpress-cjjs/issues) page.